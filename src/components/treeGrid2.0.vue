            ...
            <tbody>
                <tr v-for="(item,index) in initItems" :key="item.id" v-show="show(item)" :class="{'child-tr':item.parent}">
                    <td v-for="(column,snum) in columns" :key="column.key" :style=tdStyle(column)>
                        <label v-if="column.type === 'selection'">
                            <input type="checkbox" :value="item" v-model="checkGroup" @click="handleCheckClick(item,$event,index)">
                        </label>
                        ...
                        <label @click="toggle(index,item)" v-if="!column.type">
                            <span v-if='snum==iconRow()'>
                                ...
                                <span v-if="item.children&&item.children.length>0" >{{!item.expanded?'+':'-'}}</span>
...
export default {
    ...
    computed: {
        tableWidth() {
            return this.tdsWidth > this.screenWidth && this.screenWidth > 0 ? this.screenWidth + 'px' : '95%'
        }
    },
    watch: {
        screenWidth(val) {
